...
export default class extends Controller {
    ...
    static values = { prayers: Object, zone: String };
    ...
    async loadPrayerTimes() {
        try {
            ...
            const cached = await prayerTimesDB.getToday(this.zone);
            ...
            if (cached && cached.today) {
                ...
                await this.fetchAndCacheYearly();
            }
        } catch (error) {
    ...
    async fetchAndCacheYearly() {
        try {
            const year = new Date().getFullYear();
            const hasCache = await prayerTimesDB.hasCurrentYear(this.zone);

            if (hasCache) return;

            const response = await fetch(`/api/yearly/${this.zone}/${year}`);
            if (!response.ok) throw new Error("Failed to fetch yearly data");

            const data = await response.json();
            await prayerTimesDB.save(this.zone, year, data);
