...
export default class extends Controller {
    ...
    handleTimeFormatChange(event) {
        ...
        // Re-render the clock with new format
        this.element.innerHTML = "";
        this.appendHtml();
        this.updateClock();
    }
    ...
    appendHtml() {
        ...
        const is24Hour = this.timeFormat === "24";
        ...
        <div class="flex items-center justify-center gap-1 md:gap-2">
          ...
          ${is24Hour ? "" : '<span data-clock-target="ampm" class="text-xl md:text-2xl lg:text-3xl font-bold ml-2 text-base-content/50 self-end mb-1 md:mb-2">AM</span>'}
        </div>
      `,
            );
        } else if (isMini) {
        ...
        <div class="flex items-center gap-1 text-lg md:text-xl font-bold">
          ...
          <span data-clock-target="min">00</span>
          ${is24Hour ? "" : '<span data-clock-target="ampm" class="text-sm ml-1 opacity-70">AM</span>'}
        </div>
      `,
            );
        } else {
        ...
        <ul>
          ...
          ${is24Hour ? "" : '<li id="ampm" class="ml-2" data-clock-target="ampm">AM</li>'}
        </ul>
      `,
            );
        }
